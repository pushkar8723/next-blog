...
const projects = [
  ...
  {
    title: "Project Two",
    description: "An open-source library for building accessible React components.",
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-16 sm:px-6 lg:py-24">
      ...
      <section className="mb-20">
        <h1 className="text-4xl font-bold tracking-tight text-foreground sm:text-5xl">{siteConfig.name}</h1>
        <p className="mt-2 text-xl text-primary">{siteConfig.title}</p>
        <p className="mt-6 max-w-2xl text-lg leading-relaxed text-muted-foreground">{siteConfig.description}</p>
      ...
      <section className="mb-20">
        <div className="mb-8 flex items-center justify-between">
          <h2 className="text-2xl font-semibold tracking-tight text-foreground">Recent Writing</h2>
          <Link href="/blog">
        ...
        {posts.length > 0 ? (
            ...
            {posts.map((post) => (
              <Link key={post.slug} href={`/blog/${post.slug}`} className="group block">
                <article className="flex flex-col gap-2 rounded-lg border border-transparent p-4 transition-colors hover:border-border hover:bg-muted/50 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <h3 className="font-medium text-foreground group-hover:text-primary">{post.title}</h3>
          ...
          <p className="text-muted-foreground">
            ...
            <code className="rounded bg-muted px-1.5 py-0.5 font-mono text-sm">content/blog/</code> to get started.
          </p>
        )}
      </section>
      ...
      <section>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {projects.map((project) => (
            <Card key={project.title} className="group transition-colors hover:border-primary/50">
              <CardHeader>
